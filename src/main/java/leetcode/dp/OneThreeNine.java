...
public class OneThreeNine {
    ...
    public boolean wordBreak3(String s, List<String> wordDict) {
        ...
        // 定义Boolean[i]为s.subString[i,*)可否由wordDict组成
    ...
    private boolean contains(String s, int start, Set<String> wordSet, Boolean[][] memo) {
        ...
        for (int i = start; i <= s.length(); i++) {
            ...
            memo[start][i] = wordSet.contains(s.substring(start, i)) && contains(s, i, wordSet, memo);
            if (memo[start][i]) {
                return true;
            }
        }
    ...
    public boolean wordBreak(String s, int start, Set<String> wordDicts) {
        if (start >= s.length()) {
            return true;
        }
        for (int i = start; i <= s.length(); i++) {
            if (wordDicts.contains(s.substring(start, i)) && wordBreak(s, i, wordDicts)) {
                return true;
            }
        }
        return false;
